...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_str_divide_in_base.c                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: arabenst <arabenst@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/01 13:40:46 by arabenst          #+#    #+#             */
/*   Updated: 2023/04/19 12:51:51 by arabenst         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_div(char *dividend, char *divisor, char *base)
{
	...
	while (ft_strnumcmp(count, dividend, base) <= 0)
	{
		ft_replace_ptr((void **)&count,
			(void *)ft_str_add_in_base(count, divisor, base));
		ft_replace_ptr((void **)&quotient,
			(void *)ft_str_add_in_base(quotient, one, base));
		if (!quotient || !count)
			return (free(quotient), free(count), free(one), NULL);
	}
	ft_replace_ptr((void **)&quotient,
		(void *)ft_str_subtract_in_base(quotient, one, base));
...
char	*ft_str_divide_in_base(char *dividend, char *divisor, char *base)
{
	...
	dividend = ft_str_abs_in_base(dividend, base);
	divisor = ft_str_abs_in_base(divisor, base);
	if (!dividend || !divisor)
		return (NULL);
	if (sign_dd == sign_dv)
